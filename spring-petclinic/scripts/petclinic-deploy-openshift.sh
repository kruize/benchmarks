...
function createInstances() {
	...
	for(( inst=0; inst<${SERVER_INSTANCES}; inst++ ))
        ...
        for(( inst=0; inst<${SERVER_INSTANCES}; inst++ ))
        do
                sed 's/PETCLINIC_DEPLOYMENT_NAME/petclinic-sample-'$inst'/g' $MANIFESTS_DIR/petclinic-template.yaml > $MANIFESTS_DIR/petclinic-$inst.yaml
                sed -i 's/PETCLINIC_SERVICE_NAME/petclinic-service-'$inst'/g' $MANIFESTS_DIR/petclinic-$inst.yaml
                sed -i 's/PETCLINIC_APP/petclinic-app-'$inst'/g' $MANIFESTS_DIR/petclinic-$inst.yaml
                sed -i 's/PETCLINIC_PORT/petclinic-port-'$inst'/g' $MANIFESTS_DIR/petclinic-$inst.yaml
