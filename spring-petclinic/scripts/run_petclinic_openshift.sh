...
function parseData() {
	...
	do
		thrp_sum=0
		wer_sum=0
		svc_apis=($(oc status | grep "petclinic" | grep port | cut -d " " -f1 | cut -d "/" -f3))
		for svc_api  in "${svc_apis[@]}"
		do
...
function runItr()
{
	...
	do
		...
		./getstats-openshift.sh $TYPE-$run $CPU_MEM_DURATION $RESULTS_runItr $BENCHMARK_SERVER petclinic &
...
function runIterations() {
	...
	do
		...
		./petclinic-deploy-openshift.sh $BENCHMARK_SERVER $NAMESPACE $MANIFESTS_DIR $RESULTS_DIR_ITR $SCALING
		fi
		# Start the load
