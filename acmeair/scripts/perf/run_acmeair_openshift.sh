...
#
### Script to perform load test on multiple instances of acmeair on openshift###
#
...
while getopts s:e:u:d:w:m:i:ra:n:-: gopts
do
	case ${gopts} in
	-)
		case "${OPTARG}" in
...
# output: Check if the acmeair application and mongodb is running and if they are not running exit from the run.
function check_app() {
...
# output: Run the jmeter load on acmeair application and store the result in log file 
...
# get the kruize recommendation for acmeair application
# input: result directory
...
function get_recommendations_from_kruize()
{
	...
	do
		...
		err_exit "Error: could not generate the recommendations for acmeair"
	done
}
...
function runIterations() {
	...
	do
		...
		# Perform warmup runs
		runItr warmup ${WARMUPS} ${RESULTS_DIR_I} ${SCALING}
		echo "Running ${MEASURES} measures for ${USERS} users" >>setup.log
		# Perform measure runs
		runItr measure ${MEASURES} ${RESULTS_DIR_I} ${SCALING}
		sleep 60
