...
function runIterations() {
	...
	do
		if [ ${RE_DEPLOY} == "true" ]; then
			${SCRIPT_REPO}/galaxies-deploy-openshift.sh -s ${BENCHMARK_SERVER} -i ${SCALING} -g ${GALAXIES_IMAGE} --cpureq=${CPU_REQ} --memreq=${MEM_REQ} --cpulim=${CPU_LIM} --memlim=${MEM_LIM} >> setup.log 
