...
function cpu_request_avg_container()
{
	...
	do
		...
		curl --silent -G -kH "Authorization: Bearer ${TOKEN}" --data-urlencode 'query=avg(kube_pod_container_resource_requests{pod=~"${DEPLOYMENT_NAME}-[^-]*-[^-]*$", container="${CONTAINER_NAME}", namespace="${NAMESPACE}", resource="cpu", unit="core"})' ${URL} | jq '[ .data.result[] | [ .value[0], .metric.namespace, .metric.pod, .value[1]|tostring] | join(";") ]' | grep "${APP_NAME}" >> ${RESULTS_DIR}/cpu-${ITER}.json
